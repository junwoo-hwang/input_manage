...
- **저장하기 전에 무엇이 바뀌는지 보여준다** — 어느 시트의 몇째 줄이
  `신규` / `수정` / `삭제` 인지 줄 전체를 펼쳐서
- `REV_INFO` 시트가 있으면 **사유를 적어야 저장된다**. 날짜(한국 시간)는
  자동으로 박히고 고칠 수 없다. `관련` 은 안 적어도 된다
...
**셋. 고칠 때마다 표를 통째로 올려보내지 않는다.** 예전에는 칸 하나 고칠
때마다 15,000행을 JSON 으로 싸서(0.7MB) 파이썬에 올리고 파이썬이 그걸 다시
표로 되돌렸다. 지금은 평소에 '고친 게 있다' 만 올리고, 저장이나 엑셀 만들기
처럼 값이 진짜 필요할 때 파이썬이 `want_full` 로 한 번 달라고 한다.
